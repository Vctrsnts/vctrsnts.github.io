...
```
usuari@maquina:~$ mount
```

Y obtendras un listado con todos los dispositivos conectados. En mi caso, la tarjeta SD, era:

```
usuari@maquina:~$ /dev/mmcblk0
```

Después de obtener esta información, solamente nos queda copiar la imagen a la SD.

#### Nota: Hay que tener una cosa en cuenta a la hora de copiar la imagen en la SD, es que toda la información que pueda haber en la SD, se perdera.

La instrucción para realizar la copia ( <em>se tiene que hacer como root</em> ) es la siguiente:

```
root@maquina:~# dd bs=4M if=2016-03-18-raspbian-jessie-lite.img of=/dev/mmcblk0
462+1 records in
463+0 records out
1941962752 bytes (1.9 GB) copied, 256.996 s, 7.6 MB/s
```

#### Nota: Como se puede ver, a la opción bs, se le asigna el valor de 4M, esta puesto así, porque si se pone la opción 1M, el volcado de la información tardaria mucho.
...
PD: Si quereis obtener más información, podeis mirar este [link](http://elinux.org/RPi_Easy_SD_Card_Setup) donde se explica más detalladamente el proceso tanto para Linux, Windows y Mac
