...
Para realizar las copias de seguridad queria usar esta imagen [RSNAPSHOT](https://hub.docker.com/r/pablokbs/rsnapshot-cron/tags?page=1&ordering=last_updated), hecha por PeladoNerd, pero no funciona. No esta compilado para ARM, le di un toque por octubre, pero supongo que por trabajo, no ha podido solucionar el problema, asi que tuve que buscarme otras opciones. Y como no, salio al rescate [LINUXSERVER](https://hub.docker.com/r/linuxserver/rsnapshot).
...
rsnapshot:
  ...
  volumes:
   - ${STORAGE}/config/rsnapshot:/config
   - ${STORAGE}/seguretat:/snapshots
   - /etc:/data/etc
...
```
usuari@maquina:~$ docker-compose up -d rsnapshot
```
...
```
usuari@maquina:~$ cd dockerCompose
usuari@maquina:~/dockerCompose $ docker-compose exec rsnapshot sh
...
```
Y con esto, ya tenemos configurado **rsnapshot**, ahora solamente hay que dejarlo que funcione y listo.
